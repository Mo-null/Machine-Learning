...
for kernel in ["linear", "poly", "rbf", "sigmoid"]:
    ...
    print(f"{kernel}: accuracy={accuracy_score(y_test, p):.4f}")
...
prob_malignant = svm.predict_proba(X_test_scaled)[:, 0]
...
cv_pipeline = Pipeline([("scaler", StandardScaler()), ("svm", SVC(kernel="rbf", random_state=42))])
...
pred_class = class_names[svm.predict(sample_scaled)[0]]
print(f"\nSample prediction -> SVM: {pred_class}")
